...
try {
  ...
  let phase, preferenceFailure, memoryFailure, errorClass;
  for (const line of String(error.stderr ?? "").split("\n")) {
    try {
      ...
      if (["memory-return-publish", "memory-return-pull", "memory-return-noop"].includes(record.phase)) phase = record.phase;
      if (["MemoryReferenceError", "MemoryIdentityError", "SessionDependencyError", "SyncConflict", "AssertionError", "TypeError"].includes(record.error)) errorClass = record.error;
  ...
  process.stderr.write(`${JSON.stringify({ result: "fail", phase, preferenceFailure, memoryFailure, errorClass, error: "NativeQualificationFailed" })}\n`);
  process.exitCode = 1;
} finally {
