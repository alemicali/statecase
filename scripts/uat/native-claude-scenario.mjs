...
const provider = createServer(async (request, response) => {
  try {
    ...
    let tool;
    if (requests === 1) tool = { name: "Read", input: { file_path: join(machine.project, stage === "source" ? "input.txt" : "artifact.txt") } };
    ...
    response.end();
  } catch { fixtureFailure ??= "provider-exception"; response.writeHead(500); response.end(); }
});
try {
  ...
  assert.deepEqual(await readdir(join(target.home, "claude")), []);
  ...
  console.log(JSON.stringify({ result: "pass", harness: version, node: process.version, backend: "in-memory-reference",
    ...
    hydrationPreviewNonMutating: true, nativeProjectPath: true, returnSync: true, encryptedObjects: remote.objectCount() }));
} catch (error) {
  console.error(JSON.stringify({ result: "fail", phase, error: error.name, fixtureFailure, requests, unexpectedRoute, sessionShape }));
  process.exitCode = 1;
} finally {
...
async function harness(machine, prompt, resume) {
  ...
  check(requests === (stage === "source" ? 5 : 3), "model-turn-count");
