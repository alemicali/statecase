...
describe("two-device encrypted synchronization (SY-001, SY-010, DR-001, WS-001, WS-003, WS-004)", () => {
  ...
  it.each([false, true])("materializes only advanced namespaces without rewriting an unchanged harness beside an independent Drop, dry-run=%s (SY-011, RT-006)", async dryRun => {
    ...
    const store = new ConfigStore(join(base, "profile")), b = await store.loadConfig();
    Object.assign(b, harnessConfig(targetHarness, join(base, "project"))); b.mappings.push(config(targetDrop).mappings[0]); await store.saveConfig(b);
    const remote = new MemoryRemote(), client = new StatecaseClient("https://remote.test", "token", remote.fetch), key = await randomKey();
    const sender = new SyncEngine(client, "vlt_test", key); await sender.push(a); await sender.pull(b); await store.saveConfig(b);
    const originalProfile = await readFile(join(store.home, "config.json"));
    ...
    const download = vi.spyOn(client, "getNamespaceObject"); let called = false;
    ...
    await receiver.pull(b, dryRun); expect(called).toBe(!dryRun);
    expect(download).toHaveBeenCalled(); expect(download.mock.calls.every(call => call[1] === a.mappings[1].namespace)).toBe(true);
    expect(await readFile(localPath)).toEqual(original); expect((await lstat(localPath)).ino).toBe(originalStat.ino);
    expect(await readFile(join(targetDrop, "brief.md"), "utf8")).toBe(dryRun ? "initial brief" : "advanced brief");
    if (dryRun) expect(await readFile(join(store.home, "config.json"))).toEqual(originalProfile);
    else {
      expect((await store.loadConfig()).applied).toEqual(b.applied);
      download.mockClear(); called = false;
      expect((await receiver.pull(b)).outcome).toBe("unchanged"); expect(called).toBe(false); expect(download).not.toHaveBeenCalled();
    }
  });
  ...
  it.each(["unchanged", "legacy", "patch", "legacy-patch", "concurrent", "concurrent-patch", "post-scan-edit", "local-edit", "incomplete-tail", "initial-tail"])("returns relative-memory history without overwriting uncaptured native work: %s (AD-MEM-011)", async (mode) => {
    ...
    if (mode === "initial-tail") await writeFile(originalPath, originalBytes + '{"pending":');
    const a = harnessConfig(harness, project), b = harnessConfig(join(root, "target-harness"), join(root, "target-project"));
    a.workspaces[0]!.sync = "git"; b.workspaces[0]!.sync = "git";
    await runFile("git", ["clone", "-q", project, b.workspaces[0]!.path]);
    ...
    const fetchRemote: typeof fetch = async (input, init) => {
      if (mode === "post-scan-edit" && !editedAfterScan && init?.method === "PUT") {
        editedAfterScan = true; await writeFile(originalPath, uncommitted);
      }
      return remote.fetch(input, init);
    };
    const engine = new SyncEngine(new StatecaseClient("https://remote.test", "token", fetchRemote), "vlt_test", key);
    await engine.push(a);
    const keys = await deriveScopeKey(key, "harness:codex:default");
    expect(a.applied["harness:codex:default"]!.digests["portable-sessions/ws_test/relative.jsonl"])
      .toBe(await computeObjectId(keys.dedupKey, Buffer.from(originalBytes)));
    await engine.pull(b);
    const targetSession = join(b.mappings[0]!.path, "sessions", "statecase", "ws_test", "relative.jsonl");
    ...
    if (!["unchanged", "legacy", "patch", "legacy-patch", "concurrent", "concurrent-patch"].includes(mode)) {
      ...
      expect(await readFile(join(memory, "topic.md"), "utf8")).toBe("Before return");
